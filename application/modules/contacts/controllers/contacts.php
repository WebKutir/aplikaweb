...
class Contacts extends Controller {
	...
	{
		parent::Controller();
		$this->load->model('contact_m', '', TRUE);
	...
	function index()
	{
		...
		$data['result'] = $this->contact_m->get_all($per_page,$data['uri_segment']);
	...
	function edit($id=0){
		if ($this->input->post('name')){
			...
			$data['main']  = "form";
			$data['result']= $this->contact_m->get_detail(array(
				"id" => $id
			));
			$this->load->view('admin/templates',$data);     
		}
