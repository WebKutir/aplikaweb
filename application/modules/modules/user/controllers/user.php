...
class User extends Controller {
	...
	function index()
	{
		...
		$data['result'] = $this->user_m->get_all($per_page,$data['uri_segment'],$this->cat_id);
		...
		if ($this->form_validation->run() == TRUE){
			...
			$this->load->library('upload', $config);
				...
				if ( ! $this->upload->do_upload())
				{
					...
					$this->user_m->add($gambar,$this->cat_id);
	...
	function edit($id=0){
		...
		if ($this->form_validation->run() == TRUE){
			...
			$data['main']  = "form";
			$data['result']= $this->user_m->get_detail(array(
				"id" => $id
			));
			$this->load->view('admin/templates',$data);     
		}
	...
	function delete($id){
		//get image name
		$data['result']= $this->user_m->get_detail(array("id" => $id));
		...
		$this->user_m->delete($id); //remove from database
